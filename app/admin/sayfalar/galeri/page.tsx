...
export default function GaleriSayfaAdmin() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      .from("pages_content")
      .upsert({ page: "galeri", content: data, updated_at: new Date().toISOString() }, { onConflict: "page" });
    setSaving(false);
  ...
  return (
    <>
      ...
      <PageTitle title="GALERİ (SAYFA)" description="Galeri sayfasının üst metnini düzenleyin." />
      ...
      <form onSubmit={save} className="max-w-3xl bg-[#111111] border border-white/5 p-6 space-y-5">
        ...
        <div className="bg-[#0A0A0A] border border-[#E63946]/30 border-l-4 border-l-[#E63946] p-4 flex items-start gap-3">
          ...
          <div className="text-sm text-gray-300">
            Galeri görsellerini eklemek/düzenlemek için sol menüden{" "}
            <Link href="/admin/galeri" className="text-[#E63946] hover:underline font-semibold">Galeri</Link>{" "}
            menüsünü kullanın.
          </div>
        </div>
